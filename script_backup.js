...
async function init() {
  try {
    ...
    // Add close button handler for debug modal if it exists
...
function setupSocketEvents() {
  ...
  socket.on('offer', async ({ offer, from }) => {
    try {
      logEvent('offer-received', { from, type: offer.type });
      partnerId = from;
      isInitiator = false;
      connectionStartTime = Date.now();
      
      updateStatus('Incoming connection...');
      remoteOverlay.textContent = 'Incoming connection...';
      remoteOverlay.classList.add('connecting');
  ...
  socket.on('answer', async ({ answer, from }) => {
    try {
      ...
      if (peerConnection.signalingState === 'stable') {
        logEvent('answer-ignored', { reason: 'already-stable' });
        return;
      }
      
      logEvent('answer-received', { from });
      
      await peerConnection.setRemoteDescription(new RTCSessionDescription(answer));
      logEvent('remote-description-set', { type: 'answer' });
      
      updateStatus('Connection established with partner');
      updateConnectionUI();
    } catch (error) {
...
function setupEventListeners() {
    ...
    debugInfo.textContent = JSON.stringify(
      {
        ...
        partnerId,
        connectionAttempts,
        events: events.slice(-20) // Only include the last 20 events
      }, 
      null, 2
    );
  });
  
  // Debug modal close button handler (already added in init function to avoid errors)
  if (closeDebugBtn) {
    closeDebugBtn.addEventListener('click', () => {
  ...
  // Copy debug info - using proper variable references
  if (copyStateBtn) {
    copyStateBtn.addEventListener('click', () => {
      // Find the debug info by ID instead of using a variable
      const debugInfoElement = document.getElementById('debug-info');
      if (debugInfoElement) {
        navigator.clipboard.writeText(debugInfoElement.textContent).then(() => {
  ...
  // Run connection diagnostic - check if button exists first
...
async function createAndSendOffer() {
  if (!peerConnection) {
    logEvent('create-offer-failed', { reason: 'no-peer-connection' });
    throw new Error('No peer connection exists');
  }
  
  logEvent('create-offer-start');
  
  try {
    // Create the offer with options optimized for video chat
    const offerOptions = {
      offerToReceiveAudio: true,
      offerToReceiveVideo: true,
      voiceActivityDetection: true,
      iceRestart: connectionAttempts > 0 // Use ICE restart if this is a retry
    };
    ...
    // Wait a moment for ICE gathering before sending the offer
    ...
    if (partnerId) {
      ...
      logEvent('offer-sent', { to: partnerId });
      updateStatus('Connection offer sent');
    } else {
      throw new Error('No partner ID available');
    }
  } catch (error) {
    logEvent('create-offer-error', { error: error.message });
