...
// --- Configuration ---
const HOST_PASSWORD = '1'; // **SECURITY RISK**: Hardcoded password
...
io.on('connection', (socket) => {
  ...
  if (currentHostId) {
    ...
    // No host exists yet - make this user the host automatically
    console.log(`No host exists. Making ${socket.id} the host automatically.`);
    currentHostId = socket.id;
    io.to(socket.id).emit('auto-host', { hostId: socket.id });
